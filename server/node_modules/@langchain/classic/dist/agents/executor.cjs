...
var AgentExecutorIterator = class extends _langchain_core_load_serializable.Serializable {
	lc_namespace = [
		"langchain",
		"agents",
		"executor_iterator"
...
*/
var ExceptionTool = class extends _langchain_core_tools.Tool {
	name = "_Exception";
...
* ```
*/
var AgentExecutor = class AgentExecutor extends require_base.BaseChain {
	static lc_name() {
	...
	*/
	...
	constructor(input) {
		...
		if (_langchain_core_runnables.Runnable.isRunnable(input.agent)) {
			if (require_agent.AgentRunnableSequence.isAgentRunnableSequence(input.agent)) if (input.agent.singleAction) agent = new require_agent.RunnableSingleActionAgent({
				runnable: input.agent,
				streamRunnable: input.agent.streamRunnable
			});
			else agent = new require_agent.RunnableMultiActionAgent({
				runnable: input.agent,
				streamRunnable: input.agent.streamRunnable
			});
			else agent = new require_agent.RunnableMultiActionAgent({ runnable: input.agent });
			returnOnlyOutputs = false;
		} else {
			if (require_agent.isRunnableAgent(input.agent)) returnOnlyOutputs = false;
			agent = input.agent;
		}
		super(input);
		this.agent = agent;
	...
	async _call(inputs, runManager, config) {
		...
		while (this.shouldContinue(iterations)) {
			...
			try {
				...
				if (e instanceof _langchain_core_output_parsers.OutputParserException) {
					let observation;
					let text = e.message;
			...
			const newSteps = await Promise.all(actions.map(async (action) => {
				...
				let observation;
				try {
					observation = tool ? await tool.invoke(action.toolInput, (0, _langchain_core_runnables.patchConfig)(config, { callbacks: runManager?.getChild() })) : `${action.tool} is not a valid tool, try another one.`;
					if (typeof observation !== "string") throw new Error("Received unsupported non-string response from tool call.");
				} catch (e) {
					if (e instanceof _langchain_core_tools.ToolInputParsingException) {
