...
const getText = (html, baseUrl, summary) => {
	...
	let text = "";
	$(`${summary ? "body " : "*"}:not(style):not(script):not(svg)`).each((_i, elem) => {
...
const getHtml = async (baseUrl, h, config) => {
	...
	const contentTypeArray = (htmlResponse.headers.get("content-type") || "").split(";");
...
* ```
*/
var WebBrowser = class extends _langchain_core_tools.Tool {
	static lc_name() {
		return "WebBrowser";
	}
	get lc_namespace() {
	...
	constructor({ model, headers, embeddings, requestConfig, textSplitter }) {
		...
		this.textSplitter = textSplitter ?? new _langchain_textsplitters.RecursiveCharacterTextSplitter({
			chunkSize: 2e3,
			chunkOverlap: 200
		});
	}
	...
	async _call(inputs, runManager) {
		...
		let text;
		try {
			text = getText(await getHtml(baseUrl, this.headers, this.requestConfig), baseUrl, doSummary);
		} catch (e) {
		...
		if (doSummary) context = texts.slice(0, 4).join("\n");
			...
			const docs = texts.map((pageContent) => new _langchain_core_documents.Document({
				pageContent,
				metadata: []
			}));
			context = require_util_document.formatDocumentsAsString(await (await require_vectorstores_memory.MemoryVectorStore.fromDocuments(docs, this.embeddings)).similaritySearch(task, 4, void 0, runManager?.getChild("vectorstore")));
		}
		...
		return _langchain_core_runnables.RunnableSequence.from([this.model, new _langchain_core_output_parsers.StringOutputParser()]).invoke(input, runManager?.getChild());
	}
	name = "web-browser";
...
Object.defineProperty(exports, "webbrowser_exports", {
	enumerable: true,
	get: function() {
		return webbrowser_exports;
	}
});

//# sourceMappingURL=webbrowser.cjs.map